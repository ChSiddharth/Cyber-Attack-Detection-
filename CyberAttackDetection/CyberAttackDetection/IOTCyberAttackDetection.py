...
main = tkinter.Tk()
main.title("Trustworthy and Reliable Deep-Learning-Based Cyberattack Detection in Industrial IoT")
...
def runNeuralNetwork():
    ...
    text.insert(END,"Deep Learning Neural Network Prediction Accuracy: "+str(accuracy)+"\n\n")

    conf_matrix = confusion_matrix(y_test, y_pred) 
    plt.figure(figsize =(6, 6)) 
    ax = sns.heatmap(conf_matrix, xticklabels = labels, yticklabels = labels, annot = True, cmap="viridis" ,fmt ="g");
    ax.set_ylim([0,len(labels)])
    plt.title("Deep Learning Neural Network Classification Confusion matrix") 
...
def predict():
    ...
    for i in range(0,11):
        test[:,i] = label_encoder[i].transform(test[:,i].astype(str))        
    ...
    for i in range(len(display)):
        text.insert(END,"Test IOT Traffic: "+str(display[i])+" =====> Predicted As: "+labels[predict[i]]+"\n\n")
...
font = ('times', 14, 'bold')
title = Label(main, text='Trustworthy and Reliable Deep-Learning-Based Cyberattack Detection in Industrial IoT')
...
snButton = Button(main, text="Run Deep Learning Neural Network", command=runNeuralNetwork)
